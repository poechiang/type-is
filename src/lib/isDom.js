...
const isNode = ( node, type = 'Node' ) => window && ( dom instanceof window[ type ] );
...
for ( const [ tag, type ] of Object.entries( HtmlNameTypeMap ) ) {
    isHTMLElement[ tag ] = ( dom ) => type ? isHTMLElement( dom, type ) : ( dom.tagName && dom.tagName.toLowerCase( ) === dom.toLowerCase( ) );
}
